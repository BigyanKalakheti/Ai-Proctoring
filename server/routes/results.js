...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const allResults = await ExamResult.find(query)
      .populate('userId', 'firstName lastName email photo department')
      .populate('examId', 'title description duration')
      .sort({ createdAt: -1 });
    ...
    const filteredResults = search
      ? allResults.filter(result =>
          result.userId.firstName.toLowerCase().includes(search) ||
          result.userId.lastName.toLowerCase().includes(search) ||
          result.userId.email.toLowerCase().includes(search) ||
          result.examId.title.toLowerCase().includes(search)
        )
      : allResults;
